...
DEBUG_MODE = True
def print_debug(s):
	if (DEBUG_MODE): printer.pretty_print(s)
...
class FoundationData(object):
	...
	# Insert #
	##########
	def insert_statement(self, table_name, values):
		'''
			...
			@param values 		({str: id}): The values to insert. Keys should be column names.
		...
		table_properties = self.get_table_properties_excluding_ID(table_name)
		self.commit(insert, table_properties, values)
		printer.pretty_print_positive(insert)
	...
	def get_rows(self, first_table_name, second_table_name, join_on_statement, condition, upperbound, lowerbound):
		'''
			@param first_table_name 	(str): the name of the table
			@param second_table_name 	(str): the second table name if a JOIN is needed
			@param join_on_statement 	(str): a stement indicating the properties in the DB on which to perform the join
		...
		'''
		# If we have a second table then we need a join statement.
		assert (second_table_name != None and join_on_statement != None) or (second_table_name == None and join_on_statement == None)
		join_on_statement = join_on_statement.replace('%1', 'F').replace('%2', 'S') if join_on_statement != None else None
		select = 'SELECT * from ' + first_table_name if second_table_name == None else 'SELECT * from ' + first_table_name + ' as F JOIN ' + second_table_name + ' as S on ' + join_on_statement
		...
		if upperbound != None and lowerbound != None:
			select = select + ' LIMIT ' + str(lowerbound) + ', ' + str(upperbound) 

		results = self.__execute(select).fetchall()
		return results

	def select_all(self, first_table_name, condition=None, second_table_name = None, join_on_statement=None, upperbound=None, lowerbound=0):
		'''
		...
		'''
		rows = self.get_rows(first_table_name, second_table_name, join_on_statement, condition, upperbound, lowerbound)
		columns = self.get_table_properties(first_table_name)
...
def order_values_for_properties(values, properties):
		...
		'''
		printer.pretty_print_error("Values: " + str(values))
		printer.pretty_print_error("properties: " + str(properties))
		new_values = []
		for the_property in properties:
